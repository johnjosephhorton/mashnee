...
In any comparables analysis, perhaps the most important factor is finding properties is truly comparable locations.
...
However, it matters a great deal what the status of that land is---some properties may have land that might be sub-divided, creating another (valuable) house lot;
...
The enjoyment of unbuildable land in turn depends on factors not captured in a model, such as the shape, setting, views, what it abutts and so on.
...
To begin, we simply use square footage.
...
\subsection{Predicting price from square feet of living space}
